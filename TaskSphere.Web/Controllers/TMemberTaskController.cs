...
namespace TaskSphere.Web.Controllers
{
    ...
    public class TMemberTaskController(TMemberTaskRepo memberTMemberTask, 
        ProjectRepo projectRepo, UserRepo userRepo, 
        CommentRepo commentRepo, AttachmentRepo attachmentRepo, CurrentUserHelper currentUserHelper, NotificationRepo notificationRepo) : Controller
    {
       
        public IActionResult Index(string searchTerm, string priority)
        {
            var result = memberTMemberTask.GetAllTasks();
            if (result.HasError)
            {
        ...
        {
            var result = memberTMemberTask.UpdateTaskStatus(id, status);
        ...
        {
            var result = memberTMemberTask.GetTaskById(id);
        ...
        {
            var result = notificationRepo.MarkAllAsRead();
            if (result.HasError) return BadRequest(result.Message);
            return Ok();
        }
